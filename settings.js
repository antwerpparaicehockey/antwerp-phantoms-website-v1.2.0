...
// Version 1.2.0 publishing: create one permanent content file for GitHub Pages.
...
function publicationSource(currentData) {
  const clean = structuredClone(currentData);
  clean.version = "1.2.0";
  return `// Antwerp Phantoms permanent website content.\n// Generated through Settings > Publish.\nwindow.PUBLISHED_SITE_DATA = ${JSON.stringify(clean, null, 2)};\n`;
}

const downloadPublishedDataButton = document.getElementById("downloadPublishedData");
if (downloadPublishedDataButton) {
  downloadPublishedDataButton.addEventListener("click", () => {
    try {
      // Persist the latest form-backed state before creating the publication file.
      window.saveSiteData(data);
      downloadTextFile("published-data.js", publicationSource(data), "text/javascript");
      showSecurityMessage(document.getElementById("publishMessage"), "Publication file created. Replace the existing published-data.js in the website folder.", "success");
    } catch (error) {
      console.error("Could not create publication file", error);
      showSecurityMessage(document.getElementById("publishMessage"), "The publication file could not be created.", "error");
    }
  });
}

const downloadBackupJsonButton = document.getElementById("downloadBackupJson");
if (downloadBackupJsonButton) {
  downloadBackupJsonButton.addEventListener("click", () => {
    downloadTextFile(`antwerp-phantoms-content-backup-${new Date().toISOString().slice(0,10)}.json`, JSON.stringify(data, null, 2), "application/json");
    showSecurityMessage(document.getElementById("publishMessage"), "Editable backup downloaded.", "success");
  });
}
...
if (restoreBackupButton) {
  restoreBackupButton.addEventListener("click", async () => {
    ...
    if (!file) {
      showSecurityMessage(document.getElementById("publishMessage"), "Select a JSON backup first.", "error");
      return;
    }
    try {
      const restored = JSON.parse(await file.text());
      data = mergeSiteData(restored);
      window.saveSiteData(data);
      showSecurityMessage(document.getElementById("publishMessage"), "Backup restored. The Settings page will reload.", "success");
      setTimeout(() => location.reload(), 500);
    } catch (error) {
      console.error("Could not restore backup", error);
      showSecurityMessage(document.getElementById("publishMessage"), "This is not a valid Antwerp Phantoms backup.", "error");
    }
  });
}
